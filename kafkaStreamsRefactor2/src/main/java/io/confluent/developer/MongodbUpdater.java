...
public class MongodbUpdater {
    ...
    public void updateField(SpecificRecord record) {
        if (record instanceof shoe_customers) {
            updateCustomer((shoe_customers) record);
        } else if (record instanceof shoe_product) {
            updateProduct((shoe_product) record);
        } else throw new RuntimeException("Update field not implemented for this type of record " + record.getClass());
    }

    private void updateCustomer(shoe_customers customer) {
    ...
    private void updateProduct(shoe_product product) {
        ...
        Bson update = Updates.combine(Updates.set("product", getProductMap(product)));
        // Instructs the driver not to insert a new document if none match the query
        UpdateOptions options = new UpdateOptions().upsert(false);
